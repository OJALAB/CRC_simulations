...
sim_epraca_reg <- function(df, p_1_s, p_2_s, cens_frac) {
  ...
  observed_means <- df[(I1 == 1 | I2 == 1) & status == 1 & wakaty >= 2, .(mean_obs = mean(wakaty)), by = .(kod)]
  observed_means[is.nan(mean_obs), mean_obs := mean(df[(I1 == 1 | I2 == 1) & status == 1 & wakaty >= 2, wakaty])]
  df <- merge(df, observed_means, by = "kod", all.x = TRUE, sort = FALSE)
...
calculate_metrics_epraca_reg <- function(df_results) {
  
  metrics_list <- list()
  
  for (est in c("est_total_wakaty", "est_total_wakaty_brglm", "est_total_wakaty_reg", "est_total_wakaty_brglm_reg")) {
    error <- df_results[[est]] - df_results[["true_total_wakaty"]]
